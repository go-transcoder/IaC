...
module "s3_lambda" {
  ...
  batch_job_definition = var.batch_config.transcode_task_definition_name
}
...
resource "local_file" "tf_ansible_vars_file_new" {
  content  = <<-DOC
    ...
    tf_transcode_job_definition_name: ${var.batch_config.transcode_task_definition_name}
    tf_transcode_job_definition_execution_role: ${module.batch.execution_role}
    DOC
